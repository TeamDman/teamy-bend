...
- Fixed allocation bundles: constructor payloads (`tb_construct`), closure
  ...
  word-lowered joins), the `FID_IO_EMIT` node, and the 32-node `tb_word` value.
  ...
  retains its established per-child path. Constructors, closure captures and
  `FID_IO_EMIT` still allocate synchronously.
...
is unchanged. The word-lowered `tb_c_word_join_build` path now validates a flat
heterogeneous child layout before reserving the parent and each exact child
class as one bundle. Its linked ticket supports different direct-segment
arities without a lane-local offsets array. The CPU path still creates each
word task and delegates linking to `tb_c_word_join`. A mixed-width integration
program combines a two-word direct child with a dynamically invoked closure;
CPU-off and actual-CUDA runs both print `25`, and the CUDA run requires an
offload and fork. Its retained executable passes Compute Sanitizer memcheck
with `ERROR SUMMARY: 0 errors` using `scripts/compute-sanitizer.ps1`.
...
released. Against the current source, all nine ignored hardware tests in
...
`12ccc8b8537770999718a87a6139f90482b8858ec9be7b8a346876753475ab6f`.
...
scalar rows, seven equalities and two negative controls; and the bounded
...
`e5e767cc6b545b725994e50984d01d69091bface`; its pre-existing dirty
paths are unchanged and no Poche source was edited. The full 431,800-state graph
was not rerun and keeps its earlier attribution. The current source also closes
the word-lowered join bundle; other task/closure nodes, nested allocations and
